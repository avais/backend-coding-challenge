...
app.controller("ctrlExpenses", ["$rootScope", "$scope", "Restangular", function ExpensesCtrl($rootScope, $scope, $Restangular) {
	...
	$Restangular.setBaseUrl("http://54.169.226.46:8080/alchemytech/expense");
	// Authentication 
	var date = new Date();
	var publicKey = "admin@example.com";
	var secret = "admin12345";
	var data = publicKey+date;
	var hash = CryptoJS.HmacSHA256(data,secret);
	var signature = CryptoJS.enc.Base64.stringify(hash);

	$Restangular.setDefaultHeaders({"Key":publicKey,"Signature":signature, "Time":date});

	var loadExpenses = function() {
		// Retrieve a list of expenses via REST
		$Restangular.one("getall").getList().then(function(expenses) {
			$scope.expenses = expenses;
		});
	}

	$scope.saveExpense = function() {
		$scope.newExpense.vat = vatValue;
		var expensePayload = {
		...
		if ($scope.expensesform.$valid) {
			...
			$Restangular.one("create").post(null, expensePayload).then(function(result) {
				loadExpenses();
			});
		}
	};
